...
% ── Submitted (distilled) model metrics — single source of truth ──
\newcommand{\distf}{0.9134}    % OOF macro-F1 of the distilled student
\newcommand{\distacc}{0.9326}  % OOF accuracy of the distilled student
...
$[+0.007, +0.017]$). To comply with the single-model delivery rule, we \textbf{distil} this
ensemble into one FinBERT student (Hinton et al., 2015), our \textbf{submitted single-model
solution} at OOF macro-F1 \textbf{\distf}. We additionally show that a fine-tuned \textbf{GPT-2
...
\texttt{\detokenize{â€™}} instead of an apostrophe), 16.4\% end in truncation artefacts
...
(Section~3.1) based on the \texttt{ftfy} library (Speer, 2019).
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.27\linewidth}
    ...
    Transformer max length 96 covers the corpus with headroom and avoids unnecessary padding. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{0.86\linewidth}{@{}>{\raggedright\arraybackslash}X r r@{}}
    ...
    \gmid
    \rowcolor{lightgray} raw + fix\_text (ftfy repair)       & \textbf{0.7186} & 0.0028 \\
    ...
    \bottomrule
  \end{tabularx}
  \caption{Preprocessing ablation (5-fold CV, LR + TF--IDF). Minimal intervention wins: encoding
  repair (shaded) edges out raw text, while every aggressive pipeline hurts.}
  \label{tab:ablation}
\end{table}

\textbf{Key finding:} aggressive preprocessing \emph{destroys} signal --- ticker symbols (\$AAPL),
analyst-firm names and domain vocabulary (`downgrade', `EPS beat', `guidance raise') are
discriminative features (Go et al., 2009) --- whereas \emph{repairing} the text helps. For sparse
TF--IDF the fix\_text gain is small (+0.03pp, the vectoriser tokenises most artefacts away), but for
fine-tuned transformers, whose subword vocabularies are sensitive to corrupted bytes, the same
repair yields \textbf{+0.26pp} on FinBERT (0.9056 $\to$ 0.9082) and +0.07pp on DeBERTa-v3-large
(0.8882 $\to$ 0.8889) under the 10-fold protocol (full ablation in
\nolinkurl{tm_tests_33.ipynb}, Section~5g). The final pipeline therefore feeds the transformer
\emph{repaired raw text}: fix\_text only, no destructive cleaning.
...
\subsection{Ensemble and Knowledge Distillation (Extra Work)}
\label{sec:distill}
The backbone search leaves us with several strong, partially decorrelated encoders. As extra work
we combine them by \textbf{weighted soft-voting}: test probabilities are averaged with per-model
...
The delivery rules, however, require the final solution to be \emph{``a single pipeline with a
single classification model''} (Guidelines~5.2). Rather than discard the ensemble or break the
rule, we \textbf{distil} it (Hinton et al., 2015): the eight-model ensemble becomes the
...
itself is leakage-free. Training otherwise mirrors the 10-fold recipe of Section~5.2. A grid search
over the distillation hyperparameters ($\alpha \in \{0.5, 0.7, 0.9, 1.0\}$, $T \in \{2, 3\}$,
3-fold proxy) confirmed $\alpha{=}0.5$, $T{=}2$ as optimal; we also evaluated post-hoc per-class
probability scaling and \emph{rejected} it, as its held-out gain was within noise --- consistent
with the calibration analysis of Section~6.3. The distilled
student reaches OOF macro-F1 \textbf{\distf} (accuracy \distacc) --- recovering 44\% of the
ensemble's gain in a single 110M-parameter model, itself a statistically significant improvement
over the best conventionally-trained encoder (paired bootstrap 95\% CI: $[+0.0016, +0.0087]$).
This is the \textbf{submitted solution} (\nolinkurl{pred_33.csv}, produced by the single pipeline
in \nolinkurl{tm_final_33.ipynb}).
...
the \textbf{submitted single model} --- the FinBERT student distilled from it --- follows at
...
\begin{figure}[H]
  ...
  models: 5-fold). Green bars: ensemble teacher and submitted distilled student.}
  \label{fig:results}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}X r r r r r@{}}
    ...
    \gmid
    Ensemble, 8 encoders, optimised weights \emph{(extra)} & 0.9201 & --- & 0.9357 & 0.9097 & 0.9315 \\
    \rowcolor{lightgray} Distilled FinBERT student \textbar{} \textbf{submitted} & \textbf{\distf} & 0.0100 & \distacc & 0.9017 & 0.9263 \\
  ...
  shaded row is the \textbf{submitted} single model; rows marked \emph{(extra)} are extra work.
...
\textbf{0.871} (Bearish), \textbf{0.916} (Bullish) and \textbf{0.953} (Neutral). Errors concentrate
...
Neutral. On the held-out test set the model predicts 393 Bearish, 495 Bullish and 1{,}500 Neutral
(of 2{,}388) --- close to the training prior. Three qualitative failure modes remain: irony/sarcasm,
mixed-signal tweets (`beats EPS but misses revenue'), and truncated tweets whose decisive token was
cut off --- the ceiling for text-only models on this corpus.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.96\linewidth]{confusion_matrix_distilled.pdf}
  \caption{Confusion matrix of the submitted distilled FinBERT (normalised left, absolute counts
...
\begin{itemize}
  ...
  \item distilled student vs.\ best plain encoder: $[+0.0016, +0.0087]$ --- significant;
...
\textbf{\distf} (accuracy \distacc), a \textbf{+0.111} gain over the 0.8021 classical baseline,
