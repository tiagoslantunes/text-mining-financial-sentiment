...
\emph{presence} is mildly informative but ticker \emph{identity} is not: collapsing cashtags to a
generic token changes sparse-model F1 by only +0.2pp. Mapping tickers to company names was
therefore rejected --- it would re-introduce identity as rare tokens and, for the transformer,
shift inputs away from the pre-training distribution of FinBERT-fintwitter, which contains native
cashtags.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}X r r r r r@{}}
    ...
    \rowcolor{lightgray} Distilled FinBERT student \textbar{} \textbf{submitted} & \textbf{\distf} & 0.0093 & \distacc & 0.9032 & 0.9258 \\
...
\textbf{0.871} (Bearish), \textbf{0.917} (Bullish) and \textbf{0.954} (Neutral). Errors concentrate
...
Neutral. On the held-out test set the model predicts 390 Bearish, 492 Bullish and 1{,}506 Neutral
