...
search} (model selection via the Hugging Face Hub), end-to-end fine-tuning six encoders; the best,
a Twitter-domain \textbf{RoBERTa-large topic-sentiment} variant, reaches an out-of-fold macro-F1 of
\textbf{0.8873} --- a \textbf{+0.0852} gain over the 0.8021 baseline and our \textbf{submitted
single-model solution}. Notably, this single model also surpasses the previous RoBERTa-large-2022
candidate (0.8859) and a six-learner stacking ensemble (extra work, 0.8483), so the submission is
simultaneously our best result overall.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{0.27\linewidth}
    ...
    URL/mention boilerplate is common & URLs appear in roughly half of the tweets &
...
preprocessing destroys. Therefore, the project does \emph{not} skip preprocessing; it implements,
documents and ablates it, then justifies the final raw-text choice with evidence. Sparse baselines
still use the cleaned variants for comparison, while the submitted Twitter--RoBERTa-large relies on
its native tweet tokenizer and receives raw financial text.
...
\textbf{model selection}, we shortlist six candidate encoders and fine-tune each under the identical
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}X r r r@{}}
    ...
    \gmid
    \rowcolor{lightgray} \textbf{twitter-roberta-large-topic-sentiment-latest} & 355M & \textbf{0.8873} & \textbf{0.9090} \\
    twitter-roberta-large-2022-154m (TimeLMs)               & 355M & 0.8859 & 0.9080 \\
    deberta-v3-base (He et al., 2021)$^\dagger$             & 184M & 0.8653 & 0.8900 \\
    twitter-roberta-base-2022-154m (TimeLMs)                & 125M & 0.8550 & 0.8810 \\
    StockTwits-RoBERTa-base                                 & 125M & 0.8430 & 0.8709 \\
    twitter-roberta-base-sentiment-latest (Barbieri 2020)   & 125M & 0.8399 & 0.8691 \\
    \bottomrule
  \end{tabularx}
  \caption{Transformer backbone search (same 5-fold protocol). The shaded Twitter-domain
  RoBERTa-large topic-sentiment model is the \textbf{submitted single model}. $^\dagger$DeBERTa-v3 produced NaN losses under
...
The winner, \textbf{Twitter--RoBERTa-large topic-sentiment}, is a 355M-parameter
Twitter-domain RoBERTa-large already adapted to topic-based sentiment. We replace its original
5-label sentiment head with a fresh 3-class Bearish/Bullish/Neutral head and fine-tune the complete
network. It attains an OOF macro-F1 of \textbf{0.8873} ($\pm$0.0049), accuracy 0.9090, precision
0.8796 and recall 0.8958 --- a \textbf{+0.0852} gain over the 0.8021 frozen-embedding baseline and
\textbf{+0.0014} over the previous RoBERTa-large-2022 candidate. The final test prediction averages
the five folds' softmax probabilities. \textbf{This is the submitted single-model solution}
(\nolinkurl{pred_33.csv}, reproduced in \nolinkurl{tm_final_33.ipynb}); it is also our best result
overall (Section~6.1).
...
RoBERTa-large topic-sentiment model of Section~5.2 (0.8873). The submission is therefore the single model, which is both
...
models. The submitted single model --- fine-tuned Twitter--RoBERTa-large topic-sentiment --- tops
the ranking at macro-F1 \textbf{0.8873}, ahead of the previous RoBERTa-large-2022 candidate (0.8859),
the six-learner stack (0.8483, extra work), and every frozen-embedding model. DeBERTa-v3-base,
although a strong encoder in principle, required full fp32 to train at all (its disentangled
attention NaN-ed under bf16/fp16); even stabilised (0.8653) it trailed both large Twitter models.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\linewidth]{report_results.png}
  \caption{Top configurations ranked by 5-fold CV macro-F1.}
  \label{fig:results}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}X r r r r r@{}}
    ...
    \gmid
    \rowcolor{lightgray} Twitter-RoBERTa-large topic-sent. \textbar{} \textbf{submitted} & \textbf{0.8873} & 0.0049 & 0.9090 & 0.8796 & 0.8958 \\
    Twitter-RoBERTa-large-2022-154m                     & 0.8859 & 0.0066 & 0.9080 & 0.8754 & 0.8975 \\
    ...
    StockTwits-RoBERTa-base                             & 0.8430 & 0.0117 & 0.8709 & 0.8303 & 0.8577 \\
...
frozen-embedding plateau; (3) within fine-tuning, a large Twitter sentiment encoder gives the
biggest gain; (4) a single strong model can beat a heterogeneous stacking ensemble --- simpler and
rule-compliant.
...
misclassified as Bullish (270) or Bearish (212), reflecting the ambiguity of non-directional
financial discourse, followed by Bullish/Bearish tweets softened into Neutral. On the held-out test
set the model predicts 361 Bearish, 512 Bullish and 1{,}515 Neutral (of 2{,}388) --- close to the
training prior.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.96\linewidth]{confusion_matrix.pdf}
  \caption{Confusion matrix of the submitted fine-tuned Twitter--RoBERTa-large topic-sentiment model (normalised left,
...
fine-tuned \textbf{Twitter--RoBERTa-large topic-sentiment} model --- achieves an out-of-fold
macro-F1 of \textbf{0.8873}, a \textbf{+0.0852} gain over the 0.8021 baseline, and is also our best
result overall, surpassing a six-learner stacking ensemble (0.8483, extra work). Key contributions:
(1) empirical evidence that
