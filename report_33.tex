...
\begin{itemize}
  ...
  \item \tH{T6 --- TweetTokenizer.} \texttt{reduce\_len=True}, \texttt{strip\_handles=False}.
\end{itemize}
...
Four sparse sets on raw text: \textbf{BoW} (binary CountVectorizer, 50k);
\textbf{TF--IDF 1g} (\texttt{max\_df=0.95}, \texttt{min\_df=2}); \textbf{TF--IDF 2g} (uni+bigram,
50k); and \textbf{TF--IDF char} (char 3--5-gram, 40k).
